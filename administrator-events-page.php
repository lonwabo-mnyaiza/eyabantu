 ...
 */
    ...
    {
        $text = 'View Less';
        $query = "SELECT * FROM events ORDER BY DateModified Desc";
        $result = mysqli_query($db_conn, $query);
     }
    else 
    {
        $text = 'View All';        
        $query = "SELECT * FROM events Where Flag='Active' ORDER BY DateModified Desc";
        $result = mysqli_query($db_conn, $query);
    }
    
    if (mysqli_affected_rows($db_conn) > 0)
    {
        $event_results = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-2 col-md-1 sidebar">
          <ul class="nav nav-sidebar">
            ...
            <li class="active"><a href="administrator-events-page.php">Events</a></li>
        ...
        <div class="col-sm-9 col-sm-offset-2 col-md-10 col-md-offset-1 main">
          <h1 class="page-header">Events</h1>          
              <a href="administrator-add-page.php?category=event" class="btn btn-primary">Add</a>
              <a href="administrator-events-page.php?view=<?php echo $text; ?>" class="btn btn-warning"><?php echo $text; ?></a> 
              <a class="btn btn-success" data-toggle="modal" data-target="#remove-event-archive-confirmation">Remove All</a>
              <a class="btn btn-danger" data-toggle="modal" data-target="#clear-event-archive-confirmation">Clear Archive</a> 
              ...
              <tbody>
                  ...
                  if ($event_results != null)
                  {
                    foreach ($event_results as $event_result)
                    {
                      ...
                      echo '<td>';
                      echo $event_result['Title'];
                      echo '</td>';

                      echo '<td>';
                      echo substr($event_result['Description'], 0, 800) . '...';
                      echo '</td>';                  

                      $value = $event_result['EventID'];
                      echo '<td>';                    
                      echo '<a href="administrator-change-page.php?id='.$value.'&amp;category=event" class="btn btn-link">Update</a>';                                        
                      if ($event_result['Flag'] == 'Active')
                      {
                          echo '  <a href="administrator-delete-page.php?id='.$value.'&amp;category=event" class="btn btn-link">Remove</a>';
                      }
                      echo '<a href="administrator-view-details.php?id='.$value.'&amp;category=event" class="btn btn-link">View Details</a>';
                      echo '</td>';
      ...
      <form action="php/remove-event-all.php" method="post">
      <div class="modal fade modal-form" id="remove-event-archive-confirmation" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
      ...
      <form action="php/remove-event-archive.php" method="post">
      <div class="modal fade modal-form" id="clear-event-archive-confirmation" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">

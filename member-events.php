<?php
    ...
    $query = 'SELECT * FROM events ORDER BY DateModified'; 
           ...
           <?php  
            ...
            {          
                if ($count < 10)
                {
                    ...
                    echo '</th><th>';
                    $url = $event['URL'];
                    echo '<a style="text-decoration:none; color:#5A5A5A;" href="'.$url.'" target="_blank">';
                    echo 'read more';
                    echo '</a>';
                    echo '</th></tr><td>';                    
                    echo substr($event['Description'], 0,400) . '...';                    
                    echo '</td>';
                    $value = $event['EventID'];
                    $image_src = $event['ImageFile'];
                    echo '<td>';                       
                    echo '<img height="400" width="200" class="img-responsive" src="'.$image_src.'" data-src="js/holder.js" alt="event '.$value.' image">';
                    echo '</td>';
                    echo '</table>';
                }
                $count++;

 ...
 */
    ...
    $query1 = "SELECT Name, Description FROM services WHERE Name LIKE '$search_query' OR Description LIKE '$search_query'" or trigger_error(mysqli_error($db_conn));    
    $query2 = "SELECT Title, Description FROM news WHERE Title LIKE '$search_query' OR Description LIKE '$search_query'";    
    
    $result1 = mysqli_query($db_conn, $query1) or trigger_error(mysqli_error($db_conn));    
    if (mysqli_affected_rows($db_conn) > 0)
    {
        $service_searched_results = mysqli_fetch_all($result1, MYSQLI_ASSOC);    
    }
    else 
    {
        $success1 = false;
    ...
    if (mysqli_affected_rows($db_conn) > 0)
    {
        $news_searched_results = mysqli_fetch_all($result2, MYSQLI_ASSOC);    
    }
    else
    {
        $success2 = false;
    ...
    {
        header( 'Location: errors-public-page.php?error_value=Search' );
    }
    
     function highlightWords($words, $highligh_word)
    {
        foreach ($words as $word)
        {
            if ($word == $highligh_word)
            {
                $word = str_ireplace($word, '<span style="color:yellow;">'.$word.'</span>', $word);
            }
            $words[] = $word;
        }
...
<html>
    ...
    <body>
      ...
      <hr class="featurette-divider">     
        ...
        <h1 class="featurette-heading">Searched <span class="muted">Results.</span></h1>                
            <?php 
                echo '<h2 class="h2">Service Matches</h2>';
                foreach ($service_searched_results as $service_searched_result)
                {
                    echo '<div class="featurette">';
                    echo '<h3 class="h3">';
                    echo $service_searched_result['Name'];
                    echo '</h3>';
                    echo '<p class="lead">';
                    $desc = array();
                    $desc = explode(" ", $service_searched_result['Description']);                    
                    $highlighted_search = highlightWords($desc, $raw_search_query);
                    for ($i = 0; $i < count($highlighted_search); $i++)
                    {
                        echo $highlighted_search[$i] . ' ';
                    }
                    echo '</p>';                    
                    echo '</div>';                                    
                }
                echo '<h2 class="h2">Article Matches</h2>';
                foreach ($news_searched_results as $news_searched_result)
                {
                    echo '<div class="featurette">';
                    echo '<h3 class="h3">';
                    echo $news_searched_result['Title'];
                    echo '</h3>';
                    echo '<p class="lead">';
                    echo $news_searched_result['Description'];
                    echo '</p>';                    
                    echo '</div>';                                    
                }
            ?>

 ...
 */
    ...
    $query = 'SELECT Name, Description, ImageFile, Value FROM services' 
            or trigger_error(mysqli_error($db_conn));
    $result = mysqli_query($db_conn, $query);
    
    if (mysqli_affected_rows($db_conn) > 0)
    {
        $service_details = mysqli_fetch_all($result, MYSQLI_ASSOC);        
        mysqli_free_result($result);
    }
    
//     $query = 'SELECT Page FROM goto_page';
//     $result = mysqli_query($db_conn, $query);     
//     
//     if (mysqli_affected_rows($db_conn) > 0)
//     {
...
<html>
    <head>
        ...
        <title>Eya Bantu Services</title>
    </head>
    <body>
    ...
    <div>
        ...
        <h2 class="featurette-heading">Our <span class="muted">Services.</span></h2>
        <div class="row">
        ...
        foreach ($service_details as $service_detail)
        {
            ...
            $title = $service_detail['Name'];
            $desc = substr($service_detail['Description'], 0, 90) . '...';
            $page = 'service-desc.php';
            $image_src = $service_detail['ImageFile'];
            $value = $service_detail['Value'];
            // echo '<input type="hidden" name="service_value" value="'.$value.'" />';
            echo '<a href="'.$page.'?service_value='.$value.'" id="'.$value.'" class="btn btn-primary" data-toggle="popover" title="'.$title.'" data-placement="top" data-content="'.$desc.'">';
            echo '<img src="'.$image_src.'" data-src="js/holder.js">';
            echo '</a>';
            echo '</div>';
        }
        ?>                        
...
<!--            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>-->
        ...
        <script>
            $('#project-management-popover').popover();
            $('#renewable-energy-popover').popover();
            $('#energy-efficiency-popover').popover();
            $('#cable-fault-finding-and-location-popover').popover();
            $('#design-and-engineering-popover').popover();
            $('#power-system-studies-popover').popover();
            $('#commission-and-testing-popover').popover();
            $('#infrared-thermography-popover').popover();
            $('#transformer-oil-testing-popover').popover();
            $('#substation-and-plant-maintenance-popover').popover();
            $('#construction-management-popover').popover();
            $('#survey-popover').popover();
        </script>
        <script>
